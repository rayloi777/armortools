...
#define ITER_SAVE_SIZE (ECS_ITER_SIZE + sizeof(int) * 2)
...
static runtime_query_t *find_free_query(void) {
    for (int i = 0; i < MAX_QUERIES; i++) {
        if (!g_queries[i].valid) return &g_queries[i];
    }
    return NULL;
}

static void cache_comp_data_for_iter(runtime_query_t *q) {
...
static void save_iter_state(runtime_query_t *q) {
    if (q->iter_started && q->cached_it && q->iter_stack_top < ITER_STACK_DEPTH) {
        if (!q->iter_stack[q->iter_stack_top]) {
            q->iter_stack[q->iter_stack_top] = malloc(ITER_SAVE_SIZE);
        }
        memcpy(q->iter_stack[q->iter_stack_top], q->cached_it, ECS_ITER_SIZE);
        memcpy((char *)q->iter_stack[q->iter_stack_top] + ECS_ITER_SIZE,
                   &q->last_count, sizeof(int));
        memcpy((char *)q->iter_stack[q->iter_stack_top] + ECS_ITER_SIZE + sizeof(int),
                   &q->total_count, sizeof(int));
        q->iter_stack_top++;
    }
}

static void restore_iter_state(runtime_query_t *q) {
    if (q->iter_stack_top > 0) {
        q->iter_stack_top--;
        memcpy(q->cached_it, q->iter_stack[q->iter_stack_top], ECS_ITER_SIZE);
        memcpy(&q->last_count,
                   (char *)q->iter_stack[q->iter_stack_top] + ECS_ITER_SIZE, sizeof(int));
        memcpy(&q->total_count,
                   (char *)q->iter_stack[q->iter_stack_top] + ECS_ITER_SIZE + sizeof(int), sizeof(int));
        q->iter_started = true;
    }
}
...
int query_create(const char *filter_expr) {
    ...
    q->iter_stack_top = 0;
    memset(q->iter_stack, 0, sizeof(q->iter_stack));
...
void query_destroy(int query_id) {
    ...
    // Free iterator stack entries
    for (int i = 0; i < ITER_STACK_DEPTH; i++) {
        if (q->iter_stack[i]) {
            free(q->iter_stack[i]);
            q->iter_stack[i] = NULL;
        }
...
void *query_iter_comp_ptr(int query_id, int entity_index, int comp_index) {
    ...
    return base + entity_index * size;
}
void query_api_set_world(struct game_world_t *world) {
    (void)world; // World is accessed via engine_world_get() globally
}

static void query_api_init_impl(void) {
