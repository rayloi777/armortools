...
int minic_system_load(const char *name, const char *path) {
    ...
    void *draw_ui_ext_fn = minic_ctx_get_fn(ctx, "draw_ui_ext");
    ...
    sys->draw_ui_ext_fn = draw_ui_ext_fn;
    
    g_minic_system_count++;

    printf("[minic_system] Loaded '%s' from '%s' (step=%p, init=%p, draw=%p, draw_ui=%p, draw_ui_ext=%p)\n",
           name, path, step_fn, init_fn, draw_fn, draw_ui_fn, draw_ui_ext_fn);
...
void minic_system_unload_all(void) {
    for (int i = 0; i < g_minic_system_count; i++) {
        ...
        g_minic_systems[i].draw_ui_ext_fn = NULL;
    }
...
void minic_system_call_draw_ui_ext(void) {
    for (int i = 0; i < g_minic_system_count; i++) {
        if (g_minic_systems[i].draw_ui_ext_fn) {
            minic_call_fn(g_minic_systems[i].draw_ui_ext_fn, NULL, 0);
        }
    }
}
